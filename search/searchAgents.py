...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        isGoal = set(state[1]) == set(self.corners)
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                if nextPosition in self.corners and nextPosition not in state[1]:
                    findCorners = list(set([i for i in state[1]]))
                    findCorners.append(nextPosition)
                    successors.append(
                        ((nextPosition, findCorners), action, self.COST))
                else:
                    successors.append(
                        ((nextPosition, state[1]), action, self.COST))
...
def cornersHeuristic(state, problem):
    ...
    return max([manhattan(x, y, k, v) for k, v in corners])
...
def foodHeuristic(state, problem):
    ...
    if len(foodList) > 1:
        foodList2 = [max([(mazeDistance(foodA, foodB, currNode), foodA, foodB) for foodB in foodList]) for foodA in foodList]
        foodList3 = max(foodList2)

        return foodList3[0] + min(mazeDistance(position, foodList3[1], currNode), mazeDistance(position, foodList3[2], currNode))
        

    elif len(foodList) == 1:
        return manhattan(x, y, foodList[0][0], foodList[0][1])
