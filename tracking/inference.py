    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is not None:
            for position in self.legalPositions:
                realDistance = manhattanDistance(position, pacmanPosition)
                # Update the probability of that position: P(noisyDistance|TrueDistance) * self.beliefs[position]
                probability[position] = emissionModel[realDistance] * self.beliefs[position]
        else:
            probability[self.getJailPosition()] = float(1)

        probability.normalize()
        self.beliefs = probability
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPosition in self.legalPositions:
            newGameState = self.setGhostPosition(gameState, oldPosition)
            newDistribution = self.getPositionDistribution(newGameState)
            for newPostion, probability in newDistribution.items():
                allPossible[newPostion] += probability * self.beliefs[oldPosition]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = (self.legalPositions * ((num // len(self.legalPositions)) + 1))[:num]
    ...
    def observe(self, observation, gameState):
        ...
        """
        # reference: https://github.com/HassamSheikh/berkeley-ai-project-4/blob/master/inference.py#L197
        ...
        if noisyDistance is not None:
            ...
            for position in self.legalPositions:
                realDistance = manhattanDistance(pacmanPosition, position)
                probability[position] += emissionModel[realDistance] * self.beliefs[position]
            
            if sum(probability.values()) == 0:
                self.initializeUniformly(gameState)
            else:
                self.particles = [util.sample(probability) for _ in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        newPosDist = [self.getPositionDistribution(self.setGhostPosition(gameState, position)) for position in self.particles]
        self.particles = [util.sample(distribution) for distribution in newPosDist]
    ...
    def getBeliefDistribution(self):
        ...
        occurence = ([particle, self.particles.count(particle)] for particle in set(self.particles))
        tempt = util.Counter(occurence)
        tempt.normalize()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        num = self.numParticles
        position = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(position)
        self.particles = (position * ((num // len(position)) + 1))[:num]
    ...
    def observeState(self, gameState):
        ...
        """
        # reference: https://github.com/HassamSheikh/berkeley-ai-project-4/blob/master/inference.py#L288
        ...
        for particle in self.particles:
            ...
            for ghostIndex in range(self.numGhosts):
                if noisyDistances[ghostIndex] is None:
                    ...
                    distance = manhattanDistance(pacmanPosition, particle[ghostIndex])
                    partial *= emissionModels[ghostIndex][distance]
            probability[particle] += partial
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            tempt = list(oldParticle) # A list of ghost positions
            for ghostIndex in range(self.numGhosts):
                ghostPosition = setGhostPositions(gameState, tempt)
                newPosDist = getPositionDistributionForGhost(ghostPosition, ghostIndex, self.ghostAgents[ghostIndex])
                tempt[ghostIndex] = util.sample(newPosDist)
            newParticles.append(tuple(tempt))
